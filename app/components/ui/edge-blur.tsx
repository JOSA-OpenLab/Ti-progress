...
/**
 * Cult-UI EdgeBlur — stacked backdrop-blur layers with a gradient mask.
 * Use `position: fixed` by default (viewport-level).
 * To contain it inside a parent, add `isolate transform-gpu overflow-hidden`
 * to the parent — CSS fixed positioning snaps to the nearest transform ancestor.
 */
export function EdgeBlur({ position = "bottom", height = 75 }: EdgeBlurProps) {
  const blurLayers = [0.5, 1, 2, 3, 5]

...
### Verify Before You Publish  ✅ done

Graduate a module out of a type checker's exclusion list in a second project, and in doing so pin down the discipline the whole period depends on: never quote a number to a maintainer until you have reproduced their CI, not your laptop.

**What I did**

One contribution outside Betaflight this period. PRAXIST is a 6,319-star agent framework that carries 33 legacy files excluded from its type checker as declared debt, with the documented instruction that each graduates out of the list as it is touched. Their contributing guide names this as a good first contribution, and a closed issue names `praxist/core/role_skills.py` specifically as a candidate. The module was already clean, so it could leave the exclusion list with no source change at all: a single deleted line.

The work was entirely in proving that, and it is where I got it wrong before I got it right. My first draft of the PR claimed the type checker reported 17 errors on main. It reports zero. All 17 were my own environment: I had not synced an optional dependency group, so a third of them were phantom import errors, and I was running on macOS where platform narrowing produces three more that CI never sees. In the same draft I quoted a file count from a Homebrew-installed linter rather than the version pinned in the project's lockfile, which was off by six, and test counts polluted by bytecode caches an earlier command of my own had created.

Posting "17 errors on main" to a repo whose public CI log plainly shows zero would have made a maintainer distrust every other line in that pull request, including the parts that were right. Catching it cost an hour and saved the contribution.

The lesson is narrow and permanent, and it is the same discipline behind the two Betaflight tasks above: run the project's tool through the project's lockfile, never the copy installed on your machine; reproduce the platform CI actually runs on before quoting any number; and prove an assumption is load-bearing rather than assuming it is. I checked the exclusion was real by appending a deliberate type error to the module and confirming main reports it as clean while the patch catches it.
